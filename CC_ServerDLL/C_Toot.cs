...
namespace CC_ServerDLL {
    public class C_局域网IP {
        public static void S_获取局域网IP() {
            // TODO: Implement Functionality Here
            // Get my PC IP address
            Console.WriteLine("本地IP:_{0}", GetIPAddress());
            // Get My PC MAC address
            Console.WriteLine("本地MAC:_{0}", GetMacAddress());
            // Get My PC HostName
            Console.WriteLine("本地HostName:_{0}\n\n", Dns.GetHostName());
            ...
            foreach (KeyValuePair<IPAddress, PhysicalAddress> kvp in all) {
                Console.WriteLine("IP__{0}___________ MAC__{1}\n", kvp.Key, kvp.Value);
            }
        }
